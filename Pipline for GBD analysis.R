...
for (d in c(1:length(unique(digestdata$cause)))) {
  ...
  for (m in c(1:length(measures))) {
    ...
    ggsave(plot = p3,filename = paste(analysedir,"Number of ",measureslabel[m]," of ",aimdiseaselabel[d],"for all by age_global_2021.png",sep = ''),
           width = 11,height =8)
    ggsave(plot = p3,filename = paste(analysedir,"Number of ",measureslabel[m]," of ",aimdiseaselabel[d],"for all by age_global_2021.pdf",sep = ''),
           width = 11,height =8)
...
library(ggplot2)
library(ggsci)
analysedir <- "F:/workforSCI/252/dataanalyse/GBD/results/NAFLD/over45/test/decomposition/"
dir.create(analysedir)
diseasedata <- "F:/workforSCI/252/dataanalyse/GBD/diseasedata_all/NAFLD/Super_region_NAFLD.csv"
## reading disease data
NAFLD_Global <- read.csv(file = diseasedata)

# 
age_groups1 <- c("45-49 years", "50-54 years","55-59 years","60-64 years",
                "65-69 years","70-74 years","75-79 years","80-84 years",
                "85-89 years","90-94 years","95+ years")
age_groups2 <- c("<5 years","5-9 years","10-14 years","15-19 years","20-24 years",
                         "25-29 years","30-34 years","35-39 years","40-44 years","45-49 years",
                         "50-54 years","55-59 years","60-64 years","65-69 years","70-74 years",
                         "75-79 years","80-84 years","85-89 years","90-94 years","95+ years")

## select data of people 45+
NAFLD_in_both <- subset(NAFLD_Global,
                        (NAFLD_Global$age_name %in% age_groups) &
                          NAFLD_Global$location_name == 'Global')

NAFLD_in_both$age_name <- gsub(" years", "", NAFLD_in_both$age_name)

NAFLD_in_both$age_name <- factor(NAFLD_in_both$age_name, 
...
population <- population %>% 
  dplyr::select("Location","Time", "AgeGrp", "PopMale","PopFemale","PopTotal") %>% 
  filter(Location == 'World',
         AgeGrp %in% c("45-49", "50-54", "55-59", "60-64", 
                       "65-69", "70-74", "75-79s", "80-84", 
                       "85-89", "90-94", "95-100","100+"),
         Time %in% c(1990,2021)) %>%
  rename("location_name" = "Location","year"="Time","age_name"="AgeGrp" )
...
population <- population %>%
  mutate(
    age_name = case_when(
      age_name %in% c("95-99", "100+") ~ "95+",
      TRUE ~ as.character(age_name)
    )
  ) %>%
  group_by(location_name, year, sex_name, age_name) %>%
  summarise(val = sum(val), .groups = "drop")
population$location_name <- 'Global'
population$val <- population$val*1000
## decomposition analysis
decomposition_name <- c('sex_name', 'overll_difference', 'a_effect', 'p_effect', 
                        'r_effect', 'a_percent', 'p_percent', 'r_percent')

decomposition_data <- as.data.frame(matrix(nrow = 0, ncol = length(decomposition_name)))
names(decomposition_data) <- decomposition_name

sex_n <- c("Both", "Male", "Female")

for (a in sex_n) {
  # preparation of population data
  Global_population_1990 <- population %>% 
    filter(sex_name == a & year == 1990 & location_name == 'Global') 
  ...
  Global_population_2021 <- population %>% 
    filter(sex_name == a & year == 2021 & location_name == 'Global')
  ...
  # preparation of disease data
  case_1990 <- NAFLD_in_both %>% 
    filter(year == 1990 & sex_name == a & location_name == "Global" & 
             metric_name == 'Rate' & measure_name == 'Incidence')
  
  case_2021 <- NAFLD_in_both %>% 
    filter(year == 2021 & sex_name == a & location_name == "Global" & 
             metric_name == 'Rate' & measure_name == 'Incidence')
  ...
  temp <- data.frame(
    sex_name = a,
    overll_difference = overll_differ,
    a_effect = a_effect,
    p_effect = p_effect,
    r_effect = r_effect,
    a_percent = a_percent,
...
num_1990 <- NAFLD_Global %>% 
  ...
  group_by(sex_name) %>%
  summarise(val_1990 = sum(val)) %>%
  ungroup()

num_2021 <- NAFLD_Global %>% 
  filter(age_name %in% age_groups &
           location_name == 'Global' &
           metric_name == 'Number' &
           year == 2021 &
           measure_name == 'Incidence') %>% 
  group_by(sex_name) %>%
  summarise(val_2021 = sum(val)) %>%
  ungroup()

## combination
decomposition_data <- left_join(decomposition_data, num_1990, by = 'sex_name') %>% 
  left_join(num_2021, by = 'sex_name')

# 
decomposition_data$actual_diff <- decomposition_data$val_2021 - decomposition_data$val_1990

# 
decomposition_data[, 2:8] <- lapply(decomposition_data[, 2:8], as.numeric)

## 验证分解结果
validation <- round(decomposition_data$actual_diff) == round(decomposition_data$overll_difference)
print("validation result:")
print(validation)

# saving results
write.csv(decomposition_data, paste0(analysedir,"Incidence_45plus_Global.csv"))

## 
names(decomposition_data)[2:5] <- c('Overll_difference', 'Aging', 'Population', 'Epidemiological_change')

decomposition_plot <- decomposition_data[c(1:5)] %>%
  pivot_longer(3:5, names_to = "varname", values_to = "value") %>%
  mutate(value = as.numeric(value)) %>%
  mutate(sex_name = factor(sex_name, levels = sex_n, ordered = TRUE))

# 
my_colors <- c("#FFFB73", "#33CCCC", "#FFC773")

# 
p <- ggplot(decomposition_plot, 
            aes(x = sex_name, 
                y = value / 1e5,  
                fill = varname)) +
  ...
  theme_bw() +
  labs(x = "Sex", 
       y = expression("Effect Value" ~ (10^5)), 
       fill = "Component") +
...
# saving plots
ggsave(paste0(analysedir,"Decomposition_Analysis_45plus_Global.png"), p, width = 10, height = 8)
ggsave(paste0(analysedir,"Decomposition_Analysis_45plus_Global.pdf"), p, width = 10, height = 8)
...
## select data of people 45+
NAFLD_in_both <- subset(NAFLD_Global,
                        (NAFLD_Global$age_name %in% age_groups2) &
                          NAFLD_Global$location_name == 'Global')
...
## select all age people
num_1990 <- NAFLD_Global %>% 
  filter(age_name %in% age_groups2 &
           location_name == 'Global' &
           metric_name == 'Number' &
           year == 1990 &
           measure_name == 'Incidence') %>% 
...
# 
write.csv(decomposition_data, paste0(analysedir,"Incidence_Global.csv"))
...
# saving plots
ggsave(paste0(analysedir,"Decomposition_Analysis_Global.png"), p, width = 10, height = 8)
ggsave(paste0(analysedir,"Decomposition_Analysis_Global.pdf"), p, width = 10, height = 8)
